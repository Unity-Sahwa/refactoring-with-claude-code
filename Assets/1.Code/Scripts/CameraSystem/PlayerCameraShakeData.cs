...
namespace Refactoring
{
    ...
    {
        // public인 이유: 인스펙터에 직렬화되는 값이라 프로퍼티로 감싸면 목록에서 편집할 수 없다.
        public PlayerStateType State;
        public ShakeData Shake;
    }

    // 책임: 플레이어 상태별 카메라 셰이크 수치를 담아 둔다. 타격·피격 둘 다 이 표 하나를 State로 조회한다.
    ...
    public class PlayerCameraShakeData : ScriptableObject
    {
        ...
        // 캐싱 없이 훑는 이유: 항목이 10개 안팎이라 비용이 무시할 수준이고,
        // 에디터에서 값을 고치면(플레이 중 포함) 캐시 갱신 없이 바로 반영된다.
