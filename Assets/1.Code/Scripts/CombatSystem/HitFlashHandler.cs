...
namespace Refactoring
{
    ...
    public class HitFlashHandler : MonoBehaviour
    {
        ...
        private IEnumerator FlashRoutine(GameObject target, Renderer[] renderers)
        {
            ...
            {
                Paint(renderers, _flashColor, null);   // 빨강
                yield return new WaitForSeconds(half);
                Paint(renderers, default, original);    // 원래색
                yield return new WaitForSeconds(half);
            }

            Paint(renderers, default, original); // 끝에 확실히 원복
            _running.Remove(target);
        }

        // original이 있으면 렌더러별 그 색으로, 없으면 flat 색으로 덮는다(기존 MPB 값 보존).
        private void Paint(Renderer[] renderers, Color flat, Color[] original)
        {
            for (int i = 0; i < renderers.Length; i++)
            {
                if (renderers[i] == null || !HasColor(renderers[i])) continue;
