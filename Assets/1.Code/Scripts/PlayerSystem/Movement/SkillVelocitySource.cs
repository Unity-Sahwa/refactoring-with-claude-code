...
{
    // 책임: 상태 시스템이 주는 스킬 이동 데이터를 합산 속도로 만들어 Mover에게 전달한다.
    public class SkillVelocitySource : IVelocitySource
    {
        ...
        // 지면 요철·부동소수 오차로 떠 있다고 오판하지 않을 여유
        private const float AirborneEpsilon = 0.05f;
        // ponytail: 탐지 박스 치수 하드코딩. 스킬별로 달라져야 하면 ISkillMove에 넣는다.
        private const float BoxWidth = 0.5f;
        private const float BoxDepth = 0.5f;
        private const float BoxForwardOffset = 1f;
        ...
        {
            // Direction.normalized * Speed (캐릭터 로컬 기준, 미리 계산)
        ...
        public Vector3 Evaluate(in MoveParams frame)
        {
            ...
            if (_actives.Count == 0)
            {
                // 시간 만료로 다 빠진 경우도 초기화한다.
                _skillStartY = float.NaN;
                return Vector3.zero;
            }
        ...
        private bool OverlapEnemyInFront(in MoveParams frame, float startY)
        {
            ...
            // 시작 발높이부터 현재 캡슐 중심까지를 덮는 높이
            float height = Mathf.Max(center.y - startY, 0.01f);
