...
{
    // 책임: 입력 방향으로 캐릭터 Transform을 회전한다. (CharacterController는 회전 개념이 없어 Transform을 직접 돌린다)
    // 흐름: RotateControl 이벤트로 허용을 켜고 끔 → 허용 중이고 입력 방향이 있으면 그 방향으로 Slerp
    public class CharacterRotator : IDisposable
    {
        ...
        public CharacterRotator(IPlayerStateEventSubscriber subscriber, float rotateRate, ILockOnTarget lockOnTarget)
        {
            ...
            {
                // 회전을 제어할 상태 시스템이 없으면 제약 없이 항상 허용한다.
                _canRotate = true;
            }
        }
