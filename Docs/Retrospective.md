...
### 5-13. 내가 만든 것은 내가 설명할 수 있어야 한다 (타협 없음)

이건 권장이 아니라 **선**이다. 저장소에 들어간 것 중 내가 설명하지 못하는 코드가 하나라도 있으면
그 저장소 전체가 내 것이라고 말할 수 없게 된다. 한 줄 짚고 "이건 왜 이렇게 했나요"를 물었을 때
답이 막히면, 잘 만든 나머지 전부가 같이 의심받는다. 부분이 아니라 전체가 날아간다.

그래서 다음 프로젝트에서 지킬 것:

- **붙여넣은 코드는 그 자리에서 읽고, 못 읽겠으면 안 붙인다.** 돌아간다는 것만으로 통과시키지 않는다.
- **가져온 것은 가져왔다고 적는다.** 출처(에셋 이름·글 주소·AI 생성 여부)와 내가 고친 부분을 같이 남긴다.
  숨기는 게 아니라 구분하는 것이다. 구분해 두면 남의 것을 쓴 게 흠이 되지 않는다. 안 적어야 흠이 된다.
- **설명이 안 되는 코드는 부채로 취급한다.** 지우거나, 다시 짜거나, 이해할 때까지 붙잡는다.
  "일단 돌아가니 나중에"는 나중이 오지 않는다. 5-8에서 내 것만 따로 뽑겠다고 했는데,
  뽑는 시점에 이 구분이 안 돼 있으면 뽑는 작업 자체가 불가능하다.

판단이 갈리는 부분은 없다. 이건 취향이 아니라 **내 작업물이라고 주장할 수 있느냐의 문제**다.

### 5-14. 기록·녹화·캡처는 무조건 남긴다 (안 남긴 것은 없었던 일이 된다)

이번 프로젝트에서 제일 크게 잃은 것은 코드가 아니라 **과정**이다.
망가진 상태의 화면, 고치던 중간 단계, 왜 그 방향을 버렸는지 — 지나가면 복원할 방법이 없다.
결과 코드만 남으면 "잘 됐다"는 사실은 남지만 **내가 뭘 했는지는 안 남는다.**
남이 보기에 그건 처음부터 잘 된 코드와 구분되지 않는다.

그래서 규칙을 반대로 뒤집는다. **남길지 말지 판단하지 않는다. 일단 남긴다.**

- **찍는 비용은 0에 가깝고, 못 찍은 비용은 무한이다.** 버그 화면, 물리가 튀는 장면, 셰이더 실패,
  프로파일러 수치, 에디터 설정 창 — 판단하지 말고 찍는다. 쓸모없어 보이는 것 90%를 버리는 값이
  결정적인 10%를 놓치는 값보다 압도적으로 싸다.
- **정리하려고 미루지 않는다.** 어디에 박아두든 상관없다. 아무 폴더, 파일명 그대로.
  "정리해서 넣어야지" 하는 순간 안 찍게 된다. **찍는 것과 정리하는 것은 다른 작업이고, 순서가 있다.**
- **다만 정리는 반드시 한다.** 쌓아두기만 하면 찾지 못해서 없는 것과 같아진다.
  주기를 정해(예: 작업 단위가 끝날 때) 날짜·주제·상태(전/후)로 옮긴다.
  무엇이 무엇인지 모르겠는 파일이 나오면 그건 버리게 된다. 그 손실을 줄이려면 찍을 때
  파일명에 한 단어만 붙여도 충분하다.
- **기록 대상은 화면만이 아니다.** 실패한 접근과 버린 이유, 수치 비교(전/후), 참고한 자료 주소까지
  같은 무게로 남긴다. 5-9에서 코드 밖 작업도 같이 기록하자고 했는데, 그 기록의 원재료가 이것이다.

정리하면, **기록이 없으면 한 일도 없는 일이 된다.** 이번에 그걸 확인했으므로 다음에는 반대로 한다.

### 5-15. 시스템 간 연결을 전담하는 연결(Adapter)시스템

CameraSystem이 InputSystem의 `InputActionType`(enum)을 이벤트 매개변수로 그대로 받는 것이 1-1 FAIL로 나왔다(`LockOnController.cs`, `IInputPressedProvider.OnInputPressed`).

검토한 대안 4가지:
1. 소비 시스템마다 전용 enum 만들어 매핑
2. InputSystem이 액션별 좁은 인터페이스 노출, 분기는 자기 내부에서 처리 (채택함)
3. 별도 연결(Adapter)시스템이 둘 사이를 전담
4. 전역 이벤트 채널/버스(3번의 변형)

3번은 CameraSystem·InputSystem 둘 다 안 건드려도 되지만, 연결시스템 자체가 둘 다 참조하는 예외 지위가 필요함(DISystem의 `[Inject]`처럼 1-1 규칙에 예외 항목을 추가해야 함). 4번은 3번의 변형으로, 타입 안전성이 더 낮아질 수 있음.

**연결은 없애지지 못한다:** Adapter를 쓰더라도 CameraSystem·InputSystem의 인터페이스는 그대로 남는다. "여러 시스템이 서로 inject"가 "Adapter 하나가 여러 인터페이스를 inject"로 묶일 뿐이다. 두 시스템이 정보를 주고받아야 하는 이상 결합 자체는 사라지지 않고, 결합의 위치(여러 곳/한 곳)와 강도(구체 클래스/인터페이스)만 바뀌는 것이다.

**장단점:** 장점은 각 시스템이 서로 타입을 몰라도 되어 독립성이 올라가고, 협업 시 각자 자기 시스템만 알면 되는 것. 단점은 Adapter가 모든 시스템 쌍을 다 아는 공용 지점이 되어, 협업 시 여러 담당자가 같은 파일을 건드는 병목이 될 수 있다는 것.

**근거:** `Docs/Retrospective.md` 3-3에 이미 기록된 GimmickSystem이 결합이 제일 많이 새는 곳으로 나와 있다(SaveSlotManager, SlotLoadRunner, CameraRole, CinemachineCamera 직접 참조). 연결이 퍼져 있을 때 생기는 결과의 실제 사례다.

**다음에 할 것:** 시스템이 여러 개가 되면서 서로 연결되는 지점이 늘어날 때(지금 프로젝트는 13개 시스템, InputSystem만해도 4곳 이상과 연결), 연결시스템 도입을 프로젝트 시작 시점부터 검토한다. 결합이 없어지는 것이 아니라 한 곳으로 모이는 것임을 전제로 판단한다. 이번 프로젝트는 2번(액션별 좁은 인터페이스)로 진행한다. 수정 범위가 크다.

### 5-16. 시스템 경계에서 enum·데이터 타입을 어떻게 다룰지

5-15의 좁은 인터페이스 방식을 InputActionType·GameStateType에 적용한 뒤, PlayerSystem의 `StateEventCategory`·`CloseEventType`(15개 카테고리를 CameraSystem 등 여러 시스템이 나눠 쓰는 공용 채널)에서도 같은 FAIL이 또 나왔다.

**판단:** 이번 프로젝트는 새 기능 추가가 끝난 마무리 단계라, 나올 때마다 좁은 인터페이스를 새로 만드는 비용이 남은 작업량에 안 맞다고 보고 `system-evaluation.md` 1-1에 "enum과 순수 데이터 타입(필드만 있고 동작 없는 struct/class)은 검사에서 제외한다"는 예외를 추가했다. 이미 고친 InputActionType·GameStateType 2건은 이 기준이면 고칠 필요가 없었던 작업이 된다.

**남는 문제:** enum도 그 시스템의 개념을 그대로 담고 있어서, 값이 늘면 소비 쪽 분기(`switch`, `if`)도 따라 바뀌어야 한다. 예외로 빼면 1-1이 잡아내던 이 결합이 안 잡힌다. 데이터 타입까지 좁은 인터페이스로 막으면 시스템·기능 간 연결 자체가 어려워진다는 지적도 있었다.

**다음에 할 것:** enum·데이터 타입을 시스템 경계에서 어떻게 다룰지 — 전부 예외로 둘지, 값 개수나 변경 빈도로 기준을 나눌지, 5-15의 좁은 인터페이스를 계속 쓸지 — 다음 프로젝트 시작 시점에 정한다.

### 5-17. 다음 프로젝트부터 꼭 해야할 것

- 한번 겪었으니까 이제 커밋 체계적으로 해야함. 
- 커밋마다 다 읽어야함. 그러기 위해선 커밋 단위를 짧게 하기. 가독성은 무조건 챙기기
- 예전과 속도 비교가 크긴한데 그래도 사람이 병목임. 딴 짓하지 말고 더 단축시켜보기. 1달로..
- 에이전트한테 그냥 해달라고 하지 말기. 그럼 나중에 다 고쳐야하고, 어짜피 안보게 되어있음.
- 항상 궁금해하고 찾아보는게 큰 무기인 것 같음. 그걸 적용해야 AI의 프로젝트가 아니라 내 프로젝트 아니겠나. 빠르게 공부할 수 있다는게 지금 가장 큰 무기임. 이 시간에 계속 하셈
- 규칙짜는데 시간 걸리는 건 맞음. 무조건 필요한 기능만 규칙짤 것. 규칙짜다가 산으로 가더라.
- 매일 또는 인사이트가 생기는 매순간 일기쓰기. AI한테 시켜서 쓰지 말고. 그리고 태그 남기기

### 5-18. 규칙·방법론에 대한 메모

- 사람이 손대지 않아도 되는 규칙이 제일 좋은 것임.
- 그리고 sonnet 5로 `정제된 규칙 > 새로운 에이전트에게 평가` 이런 식으로 규칙을 만들면 현재로선 제일 베스트. opus로 그짓하면 토큰 거덜남...
- 외부에 자료를 적용하지 말려고 하고 일단 체감하고 부족한 부분은 스스로 개선해 나가기. 외부에 신기하고 새롭기도 한 방법들을 찾는 것은 재밋지만 획기적인 방법이 아님. 내 `필요에 의해` 찾기를 강력히 권고함.

### 5-19. AI 활용 방식

- 커맨드 또는 자동화 방식에는 sonnet 5을 사용하고, 추가로 다른 에이전트를 통해 확인하는 과정 무조건 넣기. 확인은 많이 할수록 좋음. 다시 물으면 하나씩 빼먹더라.
- 심심할 때마다 평가 결과물이 맞는지 전체 평가 돌려보기
- 다음 프로젝트에서는 AI 반자동화. 코드 설계하듯이, 책임 분리 잘하고 유지보수 잘하는 방식으로 진행하기

### 5-20. 디버그·테스트 코드는 실제 기능 클래스와 분리한다

`LockOnTargetDetector.cs`의 `OnDrawGizmos()`가 디버그 시각화 코드인데도 실제 탐지 로직과 같은 파일에 있어, 메서드 줄 수·책임 판정(`code-evaluation.md` 1-2)에 같이 걸렸다.

**원인:** 디버그용 코드를 기능 클래스 안에 그대로 넣었다.

**다음에 할 것:** 디버그·테스트 코드는 별도 파일로 분리해 실제 기능 클래스에 영향이 없게 한다.

### 5-21. AI 시대의 평가 기준

- 커밋 양·작업 복잡도는 더 이상 볼 필요 없음. AI가 양은 얼마든지 뽑아냄.
- 내 쓸모와 AI 쓸모를 나눠서 봐야 함. 마인드맵으로 그려보는 것도 방법.
- 많이 써보면서 불편한 지점을 직접 느껴야 함. 안 써보면 뭐가 문제인지 모름.
- 결과물이 유지보수 쉬운지 따져야 함. 사람이 다시 만질 때 피로한지가 기준.
- AI 쓸모는 자동화용 규칙, 그리고 사용자 문제 해결을 위한 서칭·판단 능력. 이 둘로 갈림.

### 5-22. git-commit 변경사항 수집 신뢰도

- git-commit 변경사항 수집하는 것의 신뢰도가 떨어짐. 유지보수가 필요함.

### 5-23. 병목사항 기록

- 경험하면서 발생하는 병목사항을을 기록해야함. 스치듯 메모하듯이 적기만 하면 나중에 모여서 자산이 될 것임.

### 5-24. 언어 확장 구조

- 언어가 확장불가능한 상태로 유지되어 있으니까 다음에 구현할 때 유지가능하도록 미리 설정하기

### 5-25. 데이터 미리 SO로 뺄지 기준 정하기

- 문제 예시로 레이어마스크가 이곳저곳에 박혀있는데, 미리 SO로 경로를 확정해야 클래스간의 연결성 확보가 편함

### 5-28. 규칙 위반과 주석

만약 규칙을 위반하는 코드들이 존재한다면 주석으로 왜 위반했는지 표현해주기. 그렇지 않으면 나중에 봤을 때, 왜 규칙 위반했는지 모르니 그냥 수정할 수 밖에 없음.
타당한 이유를 대고 바꾸면 안될 이유 적기
규칙에다가 예외들을 계속 적지 말고, 그냥 주석에다가 예외를 적기. 규칙 FAIL일때 주석이 있는지 확인하는 절차로 예외상황인지 파악시키기
...
- 메모저장하는 커맨드 만들 것. (내가 작성한 글을 형식에 맞게, 작동원리에 맞게 저장시켜머리는 것. 메모장이라고 보면 됨)
- 커밋을 조금만 모아도 한파일에서 2가지 이상의 행동이 겹칠 수 있음. 그러므로 커밋&이슈 자동화는 철저하게 만드는게 좋을듯함. 그리고 커밋할때마다 조금씩 틀이 달라지는 것 같음. 커밋 커맨드 뿐만아니라 다른 커맨드도 프레임이 조금씩 달라지는 듯함. 어떻게 강제할까

### D-3. 매프레임 GetComponent 캐싱

만약 매프레임 getcomponent를 넘겨야하는 상황이 있다면, 해당 대상의 Awake 시점에 어딘가의 저장소에 넘기도록 해서 캐시를 하는 방식 좋은 것 같음. 그러면 매프레임 어디선가 getcomponent 하는 행위 낭비를 개선할 수 있을 것 같음. 매우 좋은 아이디어

### 5-29. 규칙 파일은 참조만 두고 세부는 필요할 때만 읽기

규칙 md 파일에 세부 내용을 다 적지 않고, "이런 상황이면 이 규칙 참조"처럼 트리거 조건과 위치만 적어두는 방식.
Skill 구조와 같다. Skill도 설명(트리거 조건)만 항상 노출되고, 해당 상황일 때만 본문 전체를 불러온다.

**효과:** 매번 규칙 파일 전체를 컨텍스트에 담지 않아도 되어 토큰이 절약된다.

### 5-27. CLAUDE 규칙과 작업 공간에 대한 메모

- CLAUDE 규칙도 책임 분리 형식으로 짤 것.
- 어떤식으로 일부 자동화 되어야하는지 알았음.
- 같은 공간에서 작업하지 말기.
- 결과물로 남는건 2번 확인하기.
- 가독성과 읽기 쉬운 글자수 유지.
- 시키지 않은건 절대 안하도록 시스템을 만들 것. 오버하는게 더 검사하기 까다로움. 규칙을 추가해나가는게 정석.
